...
class ZScoreSustainData(AbstractSustainData):
    ...
    def reindex(self, index):
        ...
        return ZScoreSustainData(self.data[idx,], self.__numStages)
    ...
    def _optimise_parameters(self, sustainData, S_init, f_init, rng):
        ...
        for s in order_seq:
            ...
            for i in order_bio:
                ...
                # Safely extract scalars for np.arange
                lb = np.ravel(move_event_to_lower_bound)[0]
                ub = np.ravel(move_event_to_upper_bound)[0]
                if lb == ub:
                    possible_positions      = np.array([0])
                else:
                    possible_positions      = np.arange(lb, ub)
        ...
        for i in tqdm(range(n_iterations), "MCMC Iteration", n_iterations, miniters=tqdm_update_iters):
            if i > 0:
                ...
                for s in seq_order:
                    ...
                    move_event_to           = np.ravel(possible_positions[index])[0]
