...
class AbstractSustain(ABC):
    ...
    def subtype_and_stage_individuals(self, sustainData, samples_sequence, samples_f, N_samples):
        ...
        for i in range(N_samples):
            ...
            total_prob_subtype              = total_prob_subtype.reshape(nSamples, N_S)
            total_prob_subtype_norm         = total_prob_subtype        / np.tile(np.sum(total_prob_subtype, 1).reshape(nSamples, 1),        (1, N_S))
            total_prob_stage_norm           = total_prob_stage          / np.tile(np.sum(total_prob_stage, 1).reshape(nSamples, 1),          (1, nStages + 1)) #removed total_prob_subtype
        ...
        for i in range(nSamples):
            ...
            subtype_index                   = int(np.ravel(ml_subtype[i])[0])
            this_prob_stage                 = np.squeeze(prob_subtype_stage[i, :, subtype_index])
